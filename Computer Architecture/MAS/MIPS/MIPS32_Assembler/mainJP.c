...
int main(int argc, char ** argv){
    ...
    char * lineError = (char *) malloc(sizeof(char) * LINESIZE);

    int counter = 0;

    char * instructionC = (char *) malloc(sizeof(char) * 33);

    while(!feof(filePointer)){
        readLine(filePointer, line);
        memset(instructionC, 0, 33);
        ...
        case 0: // R-TYPE
            ...
            printf(">> .%s.\n", toBinaryString(instruction->opcode, OPCODESIZE));
...
char * toBinaryString(int decimal, int bitsNumber){
    ...
    char * binaryOut = (char *) malloc(sizeof(char) * (bitsNumber + 1));
...
int searchKey(hashtable_t * table, char * key){
    ...
    for(int i = 0; i < HASHTABLECAPACITY; i++){
        printf("%d", i);
        printf(" %p", table->regBank[i]);
        printf(" %s", table->regBank[i]->key);
        printf(" (%lu)", strlen(table->regBank[i]->key));
        printf(" %s", key);
        printf(" (%lu)\n", strlen(key));
...
hashtable_t * initializeTable(hashtable_t * table){
    ...
    char * myRegNames[] = { "ze", "at", "v0", "v1", "a0", "a1", "a2", "a3",
                            "t0", "t1", "t2", "t3", "t4", "t5", "t6", "t7",
                            "s0", "s1", "s2", "s3", "s4", "s5", "s6", "s7",
                            "t8", "t9", "k0", "k1", "gp", "sp", "fp", "ra" };

    regItem_t ** regBank = (regItem_t **) malloc(sizeof(regItem_t *) * 33);

    for(unsigned int i = 0; i < 32; i++){
        regBank[i] = create_item(myRegNames[i], i);
        printf("[I] %d %p %s ", i, regBank[i], regBank[i]->key);
    }
    printf("\n");
...
int setReg(instruction_t * inst, char * line, char * reg, hashtable_t * table){
    ...
    char * regName = (char *) malloc(sizeof(char) * 3);
    memset(regName, 0, 3);
...
int setImmd16(instruction_t * inst, match_t * match){
    ...
    char * immd16C = (char *) malloc(sizeof(char) * 8);
...
int setJumpAddress(instruction_t * inst, match_t * match){
    ...
    char * JAddrC = (char *) malloc(sizeof(char) * 10);
